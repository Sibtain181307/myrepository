...
export default class C2B extends Component {
  custid = null;
  newusercheck = false;
    ...
    return (
      <div>
        <Row>
          ...
          <Number Data={this.GetContact} title={t("Sender's Contact")} />

          {this.newusercheck === true ? (
            <Customer
              Data={this.GetCustomer}
              title={t("Add New Sender Customer")}
            />
          ) : null}
          {this.orderBaseDto.bizCustOdrDto.customer.isByID === true ? (
            <Checks
              Data={this.GetChecks}
              title={t("Sender's Location Checks")}
            />
          ) : null}
          {console.log(this.differentContactCheck, this.DifferentLocationCheck)}
          {this.DifferentLocationCheck === true ? (
            <Location Data={this.GetLocation} title={t("Location Sender")} />
          ) : null}
